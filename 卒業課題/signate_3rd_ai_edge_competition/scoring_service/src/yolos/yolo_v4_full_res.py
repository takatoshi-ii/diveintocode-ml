...
def get_random_data(annotation_line, input_shape, random=True, max_boxes=80, jitter=.15, hue=.3, sat=1.5, val=1.5, proc_img=True):
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5):
    ...
    for layer in range(num_layers):
        ...
        loss += (diou_loss + confidence_loss + class_loss) / (K.sum(object_mask)/mf + 1)
    return loss
...
if __name__ == "__main__":
    ...
    def _main():
        ...
        annotation_path = 'data_for_yolo_training_pedest_only.txt'
        validation_path = 'data_for_yolo_val_pedest_only.txt'
        log_dir = 'training/'
        classes_path = 'yolo_classes_pedest_only.txt'
        anchors_path = 'yolo_anchors_full_res.txt'
        ...
        if phase == 1:
            ...
            model = create_model(input_shape, anchors, num_classes, load_pretrained=True, weights_path='training/ep031-loss2.418-val_loss2.793.h5')
        ...
        clr = CyclicLR(base_lr=0.1, max_lr=1.0, step_size=5000,  mode='triangular2')
